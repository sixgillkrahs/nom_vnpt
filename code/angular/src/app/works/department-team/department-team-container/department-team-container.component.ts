...
@Component({
  selector: 'app-department-team-container',
  templateUrl: './department-team-container.component.html',
  styleUrls: ['./department-team-container.component.scss']
})
export class DepartmentTeamContainerComponent implements OnInit {
  @Input() displayDepartmentTeamContainer: boolean = false;
 ...
 // organizationLookupData: OrganizationUnitLookupDto[] = [];
  ...
  onMoveToTarget(event: any): void {
    // Handle event when items are moved to target
    //this.selectedUser.push(event.items);
    console.log('Items moved to target:', this.selectedUser);
    console.log('Items moved to target 1:', this.targetUsers);
  }

  onMoveToSource(event: any): void {
    // Handle event when items are moved to source
    console.log('Items moved to source:', event.items);
  }
